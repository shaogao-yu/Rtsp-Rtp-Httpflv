 ...
 ****************************************************************************/
    typedef struct
    {
        unsigned int bEndOfDec;                    // Used by driver
        unsigned int u32Width;                    // Decoded bitstream width
        unsigned int u32Height;                    // Decoded bitstream height
        unsigned int u32UsedBytes;                // Reported used bitstream byte in buffer. It is not accurate. The inaccuracy is less than 256 bytes
        unsigned int u32FrameNum;                    // Decoded slice number
...
//        Uint8 * pu8FrameBaseAddr_V_phy;        // frame buffer (V) if output format is yuv420
        unsigned int isDisplayOut;                // 0 -> Buffer in reorder buffer, 1 -> available buffer, -1 -> last flush frame
        unsigned int isISlice;                    // 1-> I Slice, 0 -> P slice
    ...
    typedef struct
    {
        ...
        unsigned int u32MaxWidth;                // Not used now
        unsigned int u32MaxHeight;                // Not used now
        unsigned int u32FrameBufferWidth;        // LCM buffer width (in pixel)
        unsigned int u32FrameBufferHeight;        // LCM buffer height (in pixel)
//        unsigned int u32CacheAlign;                // Not used now
        unsigned int u32Pkt_size;                // Current decoding bitstream length
        unsigned char *pu8Pkt_buf;                // Current decoding bitstream buffer address
        unsigned int pu8Display_addr[3];        // Buffer address for decoded data
        unsigned int got_picture;                // 0 -> Decoding has someting error. 1 -> decoding is OK in current bitstream
//        unsigned int u32BS_buf_sz;                // Reserved bitstream buffer size for decoding (Updated by driver in initialization)
        unsigned char * pu8BitStream_phy;            // physical address. buffer for bitstream
//        unsigned char * pu8BitStream;                // virtual address.
//        unsigned char * mb_info_phy;                // physical address for MB info (Updated by driver in initialization)
//        unsigned char * intra_pred_phy;            // Physical address for intra predication (Updated by driver in initialization)
        unsigned int u32OutputFmt;                // Decoded output format, 0-> Planar YUV420 format, 1-> Packet YUV422 foramt
        unsigned int crop_x;                    // pixel unit: crop x start point at decoded-frame (not supported now)
        unsigned int crop_y;                    // pixel unit: crop y start point at decoded-frame (not supported now)
